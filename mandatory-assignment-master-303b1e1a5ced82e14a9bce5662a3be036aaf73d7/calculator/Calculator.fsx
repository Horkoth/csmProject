...
let rec eval e =
  match e with
    ...
    | UPlusExpr(x) -> eval(x)
    | UMinusExpr(x) -> - eval(x)
    | GreaterExpr(x,y) -> eval(x) > eval(y)
    | GreaterEqualExpr(x,y) -> eval(x) >= eval(y)
    | SmallerExpr(x,y) -> eval(x) < eval(y)
    | SmallerEqualExpr(x,y) -> eval(x) <= eval(y)
    | Bool(x) when String.Equals(x,"true") -> true
    | Bool(x) when String.Equals(x,"false") -> false
*)
